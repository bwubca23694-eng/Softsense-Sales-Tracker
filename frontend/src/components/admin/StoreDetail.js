...
const StoreDetail = () => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const { data: d } = await api.get(`/analytics/store/${storeId}`, { params });
      setData(d);
      if (d.recentSubmissions?.[0]?.storeName) setStoreName(d.recentSubmissions[0].storeName);
    } catch { toast.error('Failed to load store analytics'); }
  ...
  useEffect(() => {
    if (!storeName) {
      api.get('/stores/all').then(r => {
        const s = r.data.find(x => x._id === storeId);
    ...
    <div>
      <div style={{ display: 'flex', alignItems: 'flex-start', gap: '1rem', marginBottom: '1.25rem', flexWrap: 'wrap' }}>
        <div style={{ flex: 1, minWidth: 0 }}>
          <button onClick={() => navigate('/admin/stores')} className="btn btn-ghost btn-sm" style={{ padding: '0.25rem 0.5rem', marginBottom: '0.25rem' }}>← Stores</button>
          <h1 className="page-title" style={{ display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
            <span style={{ fontSize: '1.4rem' }}>🏪</span> {storeName || 'Store'} Analytics
          </h1>
          <p className="page-subtitle">Detailed store performance & rankings</p>
        </div>
        ...
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: '0.75rem', marginBottom: '1.25rem' }}>
            {[
              ...
              { icon: '💰', label: 'Revenue', value: `₹${(data.overview.totalRevenue || 0).toLocaleString()}`, color: 'var(--success)' },
              { icon: '👥', label: 'Workers', value: data.byWorker.length, color: 'var(--accent-3)' },
            ].map(s => (
          ...
          {data.daily.length > 0 && (
            <div className="card" style={{ marginBottom: '1rem' }}>
              <h3 style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: '0.95rem', marginBottom: '0.875rem' }}>Daily Trend</h3>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: '1rem', marginBottom: '1rem' }}>
                  ...
                  {data.byProduct.map((p, i) => (
                    <div key={p.name} style={{ display: 'flex', alignItems: 'center', gap: '0.6rem' }}>
                      <div style={{
                        ...
                        background: i === 0 ? '#f59e0b' : i === 1 ? '#94a3b8' : i === 2 ? '#cd7c41' : 'var(--border-light)',
                        display: 'flex', alignItems: 'center', justifyContent: 'center',
                        fontSize: '0.65rem', fontWeight: 700, color: i <= 2 ? 'white' : 'var(--text-muted)'
                      }}>{i+1}</div>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.15rem' }}>
                          ...
                          <div style={{ display: 'flex', gap: '0.4rem', flexShrink: 0 }}>
                            <span className="badge badge-blue" style={{ fontSize: '0.65rem' }}>{p.totalQuantity}</span>
                            {p.totalRevenue > 0 && <span className="badge badge-green" style={{ fontSize: '0.65rem' }}>₹{p.totalRevenue.toLocaleString()}</span>}
                          </div>
                        </div>
            ...
            {/* Workers at this store */}
            {data.byWorker.length > 0 && (
              <div className="card">
                <h3 style={{ fontFamily: 'var(--font-display)', fontWeight: 700, fontSize: '0.95rem', marginBottom: '0.875rem' }}>👥 Worker Performance</h3>
                <ResponsiveContainer width="100%" height={200}>
                  <BarChart data={data.byWorker} layout="vertical">
                    ...
                    <Tooltip content={<Tip />} />
                    <Bar dataKey="totalQuantity" name="Units" fill="#8b5cf6" radius={[0,4,4,0]} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            )}
            ...
            <div className="card">
              ...
              <div className="table-wrapper" style={{ border: 'none' }}>
                <table className="table">
                  <thead><tr><th>Date</th><th>Worker</th><th>Units</th><th>Revenue</th></tr></thead>
                  <tbody>
                    {data.recentSubmissions.map(s => (
                      <tr key={s._id}>
                        <td style={{ fontSize: '0.82rem' }}>{s.date}</td>
                        <td style={{ fontSize: '0.82rem' }}>{s.workerName}</td>
          ...
          {data.overview.submissions === 0 && <div className="empty-state"><div className="icon">📊</div><p>No data in this date range</p></div>}
        </>
      )}
    </div>
  );
